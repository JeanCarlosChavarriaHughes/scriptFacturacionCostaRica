<?php
	...
	if (empty($_POST['mod_id'])) {
		...
		}  else if ($_POST['mod_moneda']==""){
			$errors[] = "Selecciona la moneda de preferencia del cliente";
		}   else if (
			!empty($_POST['mod_id']) &&
		...
		$id_moneda=intval($_POST['mod_moneda']);
		
		$id_cliente=intval($_POST['mod_id']);
		$sql="UPDATE clientes SET cedula='".$cedula."', nombre_cliente='".$nombre."', telefono_cliente='".$telefono."', email_cliente='".$email."', direccion_cliente='".$direccion."', status_cliente='".$estado."', id_moneda='".$id_moneda."' WHERE id_cliente='".$id_cliente."'";
